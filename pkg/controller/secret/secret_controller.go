...
func createPagerdutyConfig(pagerdutyRoutingKey, clusterID string) *alertmanager.PagerdutyConfig {
	...
	if !config.IsFedramp() {
		detailsMap["ocm_link"] = fmt.Sprintf("https://console.redhat.com/openshift/details/%s", clusterID)
		detailsMap["cluster_id"] = clusterID
		detailsMap["resolved"] = `{{ .Alerts.Resolved }}`
	}
